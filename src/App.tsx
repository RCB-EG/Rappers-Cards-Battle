...
const App: React.FC = () => {
    ...
    const trackObjective = useCallback((objectiveId: string, amount: number) => {
        setGameState(prevState => {
            const currentObjective = prevState.objectives[objectiveId];
            if (currentObjective && !currentObjective.claimed) {
                const newProgress = {
                    ...currentObjective,
                    progress: (currentObjective.progress || 0) + amount
                };
                return {
                    ...prevState,
                    objectives: {
    ...
    // Game State Persistence & Objective Resets
    useEffect(() => {
        const savedState = localStorage.getItem('rappersGameState');
        let loadedState: GameState;
        if (savedState) {
            loadedState = { ...initialState, ...JSON.parse(savedState) };
        } else {
            const newUserId = `user-${Math.random().toString(36).substr(2, 9)}`;
            loadedState = { ...initialState, userId: newUserId };
        }
        ...
        if (!loadedState.lastRewardClaimTime || now - loadedState.lastRewardClaimTime > oneDay) {
            setTimeout(() => setIsDailyRewardModalOpen(true), 500);
        }

        // Check for daily objectives reset
        if (!loadedState.lastDailyObjectivesReset || now - loadedState.lastDailyObjectivesReset > oneDay) {
            const newDailyObjectives: ObjectiveProgress = {};
            objectiveData.daily.forEach(obj => {
                newDailyObjectives[obj.id] = { progress: 0, claimed: false };
            });
            // Keep weekly objectives
            Object.keys(loadedState.objectives).forEach(id => {
                 if (objectiveData.weekly.some(w => w.id === id)) {
                    newDailyObjectives[id] = loadedState.objectives[id];
                }
            });
            loadedState.objectives = newDailyObjectives;
            loadedState.lastDailyObjectivesReset = now;
        }

        // Check for weekly objectives reset
        if (!loadedState.lastWeeklyObjectivesReset || now - loadedState.lastWeeklyObjectivesReset > oneWeek) {
             objectiveData.weekly.forEach(obj => {
                loadedState.objectives[obj.id] = { progress: 0, claimed: false };
            });
            loadedState.lastWeeklyObjectivesReset = now;
        }
    ...
    const handleOpenPack = useCallback((packType: PackType, isReward = false) => {
        ...
        if (packType === 'builder') trackObjective('open_builder_pack', 1);
        if (packType === 'special') trackObjective('open_special_pack', 1);
    ...
    }, [gameState.coins, isDevMode, settings.animationsOn, trackEvolutionTask, trackObjective, gameState.storage, playSfx]);
    ...
    const handleBuyCard = (card: MarketCard) => {
        ...
        trackObjective('buy_card_market', 1);
    };
    ...
    const handleListCard = (card: CardType, price: number) => {
        ...
        trackObjective('list_card_market', 1);
    };
    ...
    const handleClaimObjectiveReward = (objectiveId: string) => {
        const objective = [...objectiveData.daily, ...objectiveData.weekly].find(o => o.id === objectiveId);
        const progress = gameState.objectives[objectiveId];
        if (!objective || !progress || progress.claimed || progress.progress < objective.target) {
            return;
        }
        ...
        if (objective.reward.type === 'coins' && objective.reward.amount) {
            ...
            message = `You received ${objective.reward.amount} coins!`;
        ...
        const newObjectives = {
            ...
            [objectiveId]: { ...progress, claimed: true }
        };
        updates.objectives = newObjectives;
        
        updateGameState(updates);
        setMessageModal({ title: 'Objective Complete!', message });
    };
